...
# Overridden by ``nucleoscan.py --n-decoys``.
NSTRUCT = 1
...
PLDDT_ALPHA_REJECT_THRESHOLD = 40.0           # α-dominant
PLDDT_ALPHA_REDUCED_THRESHOLD = 60.0          # α-dominant
...
# Set per protein by ``nucleoscan.py``.
ALIGNMENT_MAP = None
PDB_N_RES = None
...
def adaptive_score_threshold(n_residues: int) -> float:
    """
    Return the length-dependent score cutoff used for candidate flags.
