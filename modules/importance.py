...
def correlate_with_nucleus(importance_df):
    """
    Pearson and Spearman correlation between importance and FES stored in
    the legacy ``nucleus_score`` field.
    """
...
def run_importance_analysis(ref_traj, cmaps, residue_df=None,
    ...
    The cmaps argument is kept for API compatibility but is not used (contact map
    is derived directly from the native structure).
