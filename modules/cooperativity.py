...
identifiers denote the intermediate-Q window. Selecting frames by a Q window
does not establish a kinetic transition-state ensemble.
"""
import numpy as np
import pandas as pd
...
def extract_tse_frames(length, use_wide=False):
    ...
    tse_mask : (n_decoys,) bool array
        Intermediate-window selection mask.
    n_tse : int
...
def compute_cooperative_signals(max_length, native_pairs, n_res):
    ...
    -------
    dict with keys:
        'q_tse': (n_res,) array (intermediate-window contact score)
        'lr_fraction': (n_res,) array
        'coop_corr': (n_res,) array
        'n_tse_frames': int (selected-window frame count)
