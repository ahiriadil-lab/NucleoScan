...
The legacy per-residue ``nucleus_score`` is then normalized to [0,1] and
...
def score_residues_sliding_window(fragment_df: pd.DataFrame,
    ...
    """
    Classic sliding-window FES stored in the legacy ``nucleus_score`` field.
...
def score_residues_zscore(fragment_df: pd.DataFrame,
                          max_length: int) -> pd.DataFrame:
    """
    Z-score variant of FES using the legacy ``nucleus_score`` field.
...
def score_residues_unified(fragment_df: pd.DataFrame,
    ...
    Axis B — Intermediate-contact: q_tse, lr_fraction and coop_corr from an
             operational Q window (legacy q_tse name; not a validated TSE)
...
def score_residues_averaged(sliding_metrics: list,
    ...
    """
    FES variant averaging across sliding/internal fragments.

    For each residue i, collect the composite scores of all fragments that
    cover position i. Compute a weighted average (weight = fragment pLDDT
    when PLDDT_WEIGHTED_AVERAGING is True, else uniform).

    Parameters
    ----------
    sliding_metrics : list of dicts from analyze_sliding_fragment
                      each has 'start', 'window_size', 'score' (or computable)
    n_residues      : int — protein length

    Returns
    -------
    DataFrame with columns: residue, nucleus_score, is_nucleus
    """
    if not sliding_metrics:
        return pd.DataFrame()
    ...
    for _, row in frag_df.iterrows():
        ...
        weight = float(row.get("plddt_mean", 50.0))
        if np.isnan(weight) or weight <= 0:
            weight = 1.0

        end = min(start + ws, n_residues)
        for ri in range(start, end):
            if PLDDT_WEIGHTED_AVERAGING:
                score_acc[ri]  += score * weight
                weight_acc[ri] += weight
            else:
                score_acc[ri]  += score
                weight_acc[ri] += 1.0
    ...
    sigma = float(np.std(norm_scores))
    threshold = mu + sigma_factor * sigma

    return pd.DataFrame({
        "residue":       np.arange(1, n_residues + 1),
        "nucleus_score": norm_scores,
        "is_nucleus":    norm_scores >= threshold,
    })
